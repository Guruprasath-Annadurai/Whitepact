...
| Evidence-write or Trust Index HTTP-call failure during `apply_governance()` blocking or silently skipping a tool call | Denial of service / Elevation of privilege | `EvidenceRepository.record()` failures propagate as exceptions (no silent swallow); a Trust Index lookup failure fails open via `TrustCheckResult.error` (an unscored model never triggers the low-trust downgrade) — consistent with the fail-open reasoning `TrustCheckResult.passes()` documents. | Mitigated for the trust lookup specifically (tested — `test_governance_trust_state.py`'s network-error case). **Gap**: an `EvidenceRepository.record()` failure (e.g. a transient DB error) currently raises and fails the whole tool call rather than degrading gracefully — stated as a gap rather than assumed handled, since no test exercises that path today. |
