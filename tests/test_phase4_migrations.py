...
async def pg_test_db() -> AsyncGenerator[str, None]:
    ...
    db_name = f"wp_mig_p4_{uuid.uuid4().hex[:12]}"
...
def test_one_canonical_alembic_head():
    """Verify exactly 1 canonical alembic head (0044) and correct revision chain."""
...
async def test_fresh_schema_to_0044_postgres(pg_test_db: str):
    """Prove fresh schema upgrades directly to 0044 on real PostgreSQL."""
