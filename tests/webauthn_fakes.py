...
from responsibleai.enterprise.security.webauthn import FLAG_AT, FLAG_UP, FLAG_UV, b64url_encode, rp_id_hash
...
def registration_blob(*, rp_id: str, origin: str, challenge: bytes, private_key: ec.EllipticCurvePrivateKey) -> tuple[str, str, str, bytes]:
    public = private_key.public_key()
    x, y = _coords(public)
    cred_id = hashlib.sha256(x + y).digest()[:16]
    cose = cbor2.dumps({1: 2, 3: -7, -1: 1, -2: x, -3: y})
    flags = FLAG_UP | FLAG_UV | FLAG_AT
    auth = rp_id_hash(rp_id) + bytes([flags]) + (0).to_bytes(4, "big") + (b"\x00" * 16) + len(cred_id).to_bytes(2, "big") + cred_id + cose
    return client_data(typ="webauthn.create", origin=origin, challenge=challenge), b64url_encode(auth), b64url_encode(cred_id), cred_id
...
def assertion_blob(
    *,
    ...
    raw_client = json.dumps({"type": "webauthn.get", "origin": origin, "challenge": b64url_encode(challenge)}, separators=(",", ":")).encode()
