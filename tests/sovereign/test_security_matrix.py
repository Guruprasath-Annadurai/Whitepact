...
from responsibleai.sovereign.graph import GraphQueryBudget, apply_graph_budget, AuthorityGraph
...
async def test_simulation_cannot_mint_grant(svc, store) -> None:
    ...
    async with store.engine.raw.connect() as conn:
        count = (
            await conn.execute(select(func.count()).select_from(governance_execution_authorizations))
        ).scalar()
...
async def test_simulation_no_consequential_counter(svc, store) -> None:
    ...
    with ZeroEffectScope():
        ...
        svc.evaluate_shadow(ctx, agent_id="agent", action_type="x", granted_action_types=frozenset({"x"}))
        assert consequential_invocation_count() == 0
