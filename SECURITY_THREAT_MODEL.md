...
| Malicious release | consumer systems | insider/pipeline attacker | release privilege | publish bytes not reviewed | signed intent, reusable trusted builder, Trusted Publishing, SBOM/attestations, exact-byte checks | consumer verification | v1.2.6 publish run, release regression/reproducibility tests | consumers must still define policy and trust GitHub's hosted build platform; provenance does not prove artifact security |
