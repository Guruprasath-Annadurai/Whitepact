...
Last reviewed: 2026-08-28 · Platform version: 1.2.3 · Author: solo maintainer (self-review, not independent — see "How to read this document")
...
| C4 Runtime authority enforcement | 2.22 | `WhitePactRuntimeGateway` + `InternalToolExecutor`/`UpstreamMCPExecutor` | `governance/gateway.py`, `governance/execution.py`, `governance/upstream_executor.py` | `tests/test_mcp_governance_dispatch.py`, `tests/test_executor_bypass_invariant.py`, `tests/test_citadel_execution_containment.py`, `tests/test_upstream_gateway.py` | ci.yml pytest | Covers this platform's own MCP dispatch path (internal tools and upstream-proxied calls alike), not direct library use; opt-in (`mcp_governance_enabled`) |
...
| C6 Execution Permit/approval binding | 2.12–2.15 | `compute_action_digest`, consumed-flag, target-fingerprint drift detection (Execution Permit v2) | `governance/approval.py`, `governance/execution.py`, `governance/upstream_executor.py` | `tests/test_approval_execution_binding.py`, `tests/test_executor_bypass_invariant.py`, `tests/test_tool_trust.py::TestExecutionPermitV2FingerprintDrift`, `tests/test_citadel_execution_containment.py` | ci.yml pytest | `ExecutionAuthorization` not cryptographically signed (correct only while in-process) |
...
| C11 Hash-chain tamper-evidence | 2.23 | `entry_hash = sha256(prev_hash + fields)`; offline-verifiable, digest-bearing bundle export as the anchoring mechanism | `db/evidence_repository.py`, `governance/evidence_bundle.py` | (chain-verify covered by governance persistence tests); `tests/test_evidence_bundle.py`; `tests/test_evidence_chain_anchoring.py` (proves a bundle digest captured before a full-chain-regeneration attack detects it, where `verify_chain()` alone cannot) | ci.yml pytest | `verify_chain()` alone does not detect full-chain recompute by an attacker with DB write access; the mitigating mechanism (bundle export) exists and is proven to detect it, but no *automated*, periodic publication to external write-once storage exists yet — a caller must export and store a bundle themselves; process-local, not cross-replica |
| C12 Fail-closed on evidence-write and pre-decision dependency failure | 2.24 (C12) | explicit catch + block (evidence-write); exception propagation before evidence is written (six other governed-pipeline dependencies) | `mcp/governance_integration.py` | `tests/test_mcp_governance_dispatch.py::TestEvidenceWriteFailsClosed`, `::TestAuthoritySubsystemCrashFailsClosed`, `tests/test_resilience_fail_closed_matrix.py` | ci.yml pytest | Trust-check (as opposed to evidence-write) failures fail *open* by asymmetric, documented design; three dependencies (`recent_violation_count()`, `recent_autonomous_action_count()`, `evidence_repo.list_recent_actions()`) remain individually untested for this property |
