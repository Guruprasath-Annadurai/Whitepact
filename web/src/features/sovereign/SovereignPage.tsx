...
export function SovereignPage() {
  return <div className="sovereign-public"><Seo title="WhitePact Sovereign | Authority analysis workbench" description="Inspect configured authority relationships and supporting evidence with WhitePact Sovereign." path="/sovereign" />
    ...
    <main>
      ...
      <section className="sov-band sov-comparison"><div><h2>Expected vs Effective</h2><p>Compare configured intent with canonical effective authority when an authenticated tenant-bound browser contract is available.</p></div><div className="comparison-rails"><article><span>Expected · configured</span><pre>{`identity: svc-payments\naction: payments.execute\npolicy: require-approval`}</pre></article><article className="is-unavailable"><span>Effective · derived</span><strong>Tenant-bound web contract required</strong><p>No effective-authority result is computed in this browser.</p></article></div></section>
      ...
      <section className="sov-final"><h2>Build authority with clarity.</h2><p>Explore capability negotiation and the development-only interface preview while tenant-bound Sovereign web contracts are completed.</p><Link className="wp-button wp-button--primary" to="/sovereign/workbench">Explore Sovereign <ArrowRight /></Link></section>
    </main><PublicFooter />
  </div>;
}

function Unavailable({ icon: Icon, title, copy }: { icon: typeof Radar; title: string; copy: string }) { return <article className="sov-unavailable"><Icon /><h2>{title}</h2><strong>Unavailable in production</strong><span>Tenant-bound web contract required</span><p>{copy}</p></article>; }
