...
function PasswordField({ value, onChange, autoComplete, invalid = false, describedBy }: { value: string; onChange: (value: string) => void; autoComplete: string; invalid?: boolean; describedBy?: string }) {
  const [visible, setVisible] = useState(false);
  return <label className="field"><span>Password</span><div className="password-field"><input value={value} onChange={(event) => onChange(event.target.value)} type={visible ? "text" : "password"} autoComplete={autoComplete} minLength={12} required aria-invalid={invalid} aria-describedby={describedBy} /><button type="button" onClick={() => setVisible(!visible)} aria-label={visible ? "Hide password" : "Show password"}>{visible ? <EyeOff aria-hidden="true" /> : <Eye aria-hidden="true" />}</button></div></label>;
}
...
export function LoginPage() {
  ...
  return <AuthLayout><Brand /><div className="auth-form"><h1>Welcome back.</h1><p>Manage your agents, policies and governance from one control plane.</p><form onSubmit={submit}><label className="field"><span>Email</span><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" required aria-invalid={Boolean(error)} aria-describedby={error ? "login-error" : undefined} /></label><PasswordField value={password} onChange={setPassword} autoComplete="current-password" />{error && <div id="login-error" className="form-error" role="alert">{error}</div>}<div className="form-links"><Link to="/forgot-password">Forgot password?</Link><Link to="/signup">Create account</Link></div><Button disabled={busy}>{busy ? <><LoaderCircle className="spin" /> Signing in…</> : "Sign in →"}</Button></form></div></AuthLayout>;
}
...
export function SignupPage() {
  ...
  return <AuthLayout reversed><Brand /><div className="auth-form auth-form--signup"><h1>Create your<br />WhitePact account<span>.</span></h1><p>Your account is verified before an organization or API key can be activated.</p><form onSubmit={submit}><label className="field"><span>Full name</span><input value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} autoComplete="name" required aria-invalid={Boolean(error)} aria-describedby={error ? "signup-error" : undefined} /></label><label className="field"><span>Work email</span><input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} autoComplete="email" required aria-invalid={Boolean(error)} aria-describedby={error ? "signup-error" : undefined} /></label><PasswordField value={form.password} onChange={(password) => setForm({ ...form, password })} autoComplete="new-password" /><ul className="password-rules" aria-label="Password requirements">{["At least 12 characters", "Uppercase and lowercase letters", "At least one number", "At least one symbol"].map((rule, i) => <li className={requirements[i] ? "met" : ""} key={rule}>{rule}</li>)}</ul><label className="checkbox"><input type="checkbox" checked={form.accepted_terms} onChange={(e) => setForm({ ...form, accepted_terms: e.target.checked })} required /><span>I agree to the <Link to="/terms" target="_blank">Terms of Service</Link> and acknowledge the <Link to="/privacy" target="_blank">Privacy Policy</Link>.</span></label>{error && <div id="signup-error" className="form-error" role="alert">{error}</div>}<Button disabled={busy || requirements.some((item) => !item)}>{busy ? "Creating…" : "Create account →"}</Button></form><p className="auth-foot">Already have an account? <Link to="/login">Sign in</Link></p></div></AuthLayout>;
}
