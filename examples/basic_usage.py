...
async def main() -> None:
    ...
    for result in suite.probe_results:
        ...
        print(
            f"[{status}] {result.probe_name:<28} score={result.overall_score:.4f}  "
        ...
        for tr in result.template_results:
            ...
            print(f"       [{tr.severity:<8}] {tr.divergence_score:.4f}  {tr.template[:60]}…{pair}")
        print()

    ix = compute_intersectional_report(suite)
    if ix.highest_risk_pair:
        print(
            f"Intersectional risk: {' × '.join(ix.highest_risk_pair)}"
