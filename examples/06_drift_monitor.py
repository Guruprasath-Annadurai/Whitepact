...
for model, provider in [("gpt-4o", "openai"), ("claude-sonnet-4", "anthropic")]:
    ...
    print(
        f"  Current    : {trend['current_score']:.1f}  7d avg: {trend['avg_7_day']:.1f}  30d avg: {trend['avg_30_day']:.1f}"
    )
    print(f"  Last 6 wks : {scores_display}")
    print()

# Drift alerts
all_alerts = [("gpt-4o", w, a) for w, a in alerts_gpt4o] + [
    ("claude-sonnet-4", w, a) for w, a in alerts_claude
]
if all_alerts:
    print(f"  Drift Alerts ({len(all_alerts)} total)")
    for model, week, alert in all_alerts:
        sev_icon = {"low": "ℹ", "medium": "⚠", "high": "⚠⚠", "critical": "🚨"}.get(
            alert.severity, "?"
        )
        dims = ", ".join(alert.affected_dimensions) or "overall"
        print(
            f"  {sev_icon} Week {week:2d} [{alert.severity.upper():8s}] {model}: {alert.delta:.1f} pts  ({dims})"
        )
else:
...
for row in monitor.all_models():
    print(
        f"  {row['model_name']:20s} {row['provider']:12s} Score: {row['overall']:.1f}  [{row['grade']}]"
    )
