...
class ConsentProofRepository:
    ...
    async def create(self, proof: ConsentProof, *, organization_id: str) -> ConsentProof:
    ...
    async def get(self, consent_id: str, *, organization_id: str) -> ConsentProof | None:
        async with self._engine.raw.connect() as conn:
            row = (
                await conn.execute(
                    select(governance_consent_proofs).where(
                        governance_consent_proofs.c.consent_id == consent_id,
                        governance_consent_proofs.c.organization_id == organization_id,
                    )
                )
            ).fetchone()
    ...
    async def revoke(
        self, consent_id: str, *, organization_id: str, revoked_by: str, reason: str | None = None
    ) -> ConsentProof:
        existing = await self.get(consent_id, organization_id=organization_id)
        if existing is None:
        ...
        updated = await self.get(consent_id, organization_id=organization_id)
