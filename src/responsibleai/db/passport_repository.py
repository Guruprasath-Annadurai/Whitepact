...
class PassportRepository:
        ...
        async with self._engine.raw.connect() as conn:
            row = (
                await conn.execute(
                    ...
                    .order_by(
                        trust_passports.c.certified.desc(), trust_passports.c.generated_at.desc()
                    )
                    .limit(1)
                )
            ).fetchone()
    ...
    def _row_to_dict(r: Any) -> dict[str, Any]:
        return {
            ...
            "hallucination_summary": json.loads(r.hallucination_summary)
            if r.hallucination_summary
            else {},
