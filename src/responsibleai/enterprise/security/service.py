...
class IdentityRateLimiter:
    """Progressive in-process throttle. Redis may assist later; it is not authority."""
    ...
    def check(self, key: str, *, limit: int, window_seconds: float) -> None:
        now = time.monotonic()
        bucket = [ts for ts in self._events[key] if now - ts < window_seconds]
...
class IdentitySecurityService:
    ...
    limiter: IdentityRateLimiter = None  # type: ignore[assignment]
    ...
    async def _notify(self, event_type: str, *, user_id: str | None = None, org_id: str | None = None, payload: dict[str, Any] | None = None) -> None:
        safe = {k: v for k, v in (payload or {}).items() if "secret" not in k.lower() and "token" not in k.lower()}
    ...
    async def _org_policy(self, org_id: str | None) -> OrgAuthPolicy:
        ...
        async with self.engine.raw.connect() as conn:
            row = (await conn.execute(select(org_security_policies).where(org_security_policies.c.org_id == org_id))).fetchone()
            sso = (await conn.execute(select(organization_sso_configs).where(organization_sso_configs.c.org_id == org_id))).fetchone()
        if row is None and sso is None:
            return OrgAuthPolicy()
        roles = json.loads(row.privileged_roles_json) if row else ["OWNER", "SECURITY_ADMIN"]
        dual = json.loads(row.dual_control_json) if row else []
        return OrgAuthPolicy(
            phishing_resistant_required=bool(row.phishing_resistant_required) if row else False,
            privileged_roles=tuple(roles),
            sso_enforcement=(sso.enforcement if sso is not None else (row.sso_enforcement if row else "SSO_OPTIONAL")),
    ...
    async def begin_webauthn(self, *, user_id: str | None, session_id: str | None, ceremony: str, org_id: str | None = None) -> dict[str, str]:
    ...
    async def _consume_challenge(self, conn: Any, *, challenge_b64: str, ceremony: str, user_id: str | None) -> bytes:
    ...
    async def finish_passkey_registration(
        ...
        *,
        ...
        if session.user_id != user_id:
            raise unauthenticated()
        decision = self.policy.evaluate_step_up(
            action=SensitiveAction.ADD_PASSKEY,
            session=session,
            role=None,
            org=await self._org_policy(session.org_id),
        )
        if session.rank() < ASSURANCE_RANK["PASSWORD"]:
            raise EnterpriseError(STEP_UP_REQUIRED, "Fresh authentication is required to add a passkey.", 401)
        try:
            async with self.engine.raw.begin() as conn:
                challenge = parse_client_data_challenge(client_data_b64)
                await self._consume_challenge(conn, challenge_b64=challenge, ceremony="register", user_id=user_id)
        ...
        return {"id": record_id, "credential_id": cred_id_b64, "display_name": display_name, "code": decision.code}
    ...
    ) -> tuple[str, SessionAssurance]:
        self.limiter.check(f"passkey:{credential_id}", limit=20, window_seconds=60)
        try:
            async with self.engine.raw.begin() as conn:
                challenge = parse_client_data_challenge(client_data_b64)
                cred = (
                ...
                user = (await conn.execute(select(web_users).where(web_users.c.id == cred.user_id))).fetchone()
                ...
                sign_count = verify_assertion(
                    ...
                    require_uv=True,
                    previous_sign_count=int(cred.sign_count or 0),
                )
                await conn.execute(
                    update(passkey_credentials)
                    .where(passkey_credentials.c.id == cred.id)
    ...
    async def remove_passkey(self, *, user_id: str, credential_row_id: str, session: SessionAssurance, grant: str | None = None) -> None:
        await self.require_step_up(session, SensitiveAction.REMOVE_PASSKEY, org_id=session.org_id, grant=grant)
        async with self.engine.raw.begin() as conn:
            remaining = (
                await conn.execute(
                    select(passkey_credentials).where(
            ...
            totp = (
                await conn.execute(select(human_totp_factors).where(human_totp_factors.c.user_id == user_id))
            ).fetchone()
            if len(remaining) <= 1 and (totp is None or totp.status != "ACTIVE"):
                org = await self._org_policy(session.org_id)
                if org.phishing_resistant_required:
                    raise EnterpriseError(LAST_AUTH_METHOD, "Cannot remove the last strong factor.", 403)
            result = await conn.execute(
                update(passkey_credentials)
                .where(
    ...
    async def start_totp(self, user_id: str) -> dict[str, str]:
        ...
        async with self.engine.raw.begin() as conn:
            existing = (await conn.execute(select(human_totp_factors).where(human_totp_factors.c.user_id == user_id))).fetchone()
            if existing is None:
                await conn.execute(
                ...
                await conn.execute(
                    ...
                    .values(pending_secret_encrypted=secret, status="PENDING" if existing.status != "ACTIVE" else existing.status)
                )
        return {"otpauth_uri": uri, "status": "PENDING"}

    async def confirm_totp(self, user_id: str, code: str) -> None:
        self.limiter.check(f"totp:{user_id}", limit=8, window_seconds=60)
        async with self.engine.raw.begin() as conn:
            row = (await conn.execute(select(human_totp_factors).where(human_totp_factors.c.user_id == user_id))).fetchone()
    ...
    async def remove_totp(self, *, user_id: str, session: SessionAssurance, grant: str | None = None) -> None:
        await self.require_step_up(session, SensitiveAction.DISABLE_MFA, org_id=session.org_id, grant=grant)
        async with self.engine.raw.begin() as conn:
            keys = (
                await conn.execute(
                    select(passkey_credentials).where(
            ...
            if not keys and org.phishing_resistant_required:
                raise EnterpriseError(SECURITY_DOWNGRADE_BLOCKED, "Cannot disable the last strong factor.", 403)
            await conn.execute(update(human_totp_factors).where(human_totp_factors.c.user_id == user_id).values(status="REMOVED"))
        await self._notify("totp_removed", user_id=user_id)

    async def issue_recovery_codes(self, user_id: str, *, session: SessionAssurance, grant: str | None = None) -> list[str]:
        await self.require_step_up(session, SensitiveAction.CHANGE_RECOVERY_METHODS, org_id=session.org_id, grant=grant)
        codes = ["".join(secrets.choice(_ALPHABET) for _ in range(_CODE_LEN)) for _ in range(_RECOVERY_CODES)]
        async with self.engine.raw.begin() as conn:
            current = (
                await conn.execute(
            ...
            await conn.execute(
                ...
                .where(recovery_code_hashes.c.user_id == user_id, recovery_code_hashes.c.consumed_at.is_(None))
    ...
    async def consume_recovery_code(self, user_id: str, code: str) -> None:
        self.limiter.check(f"recovery-code:{user_id}", limit=5, window_seconds=300)
        async with self.engine.raw.begin() as conn:
            row = (
                await conn.execute(
            ...
            result = await conn.execute(
                update(recovery_code_hashes)
                .where(recovery_code_hashes.c.id == row.id, recovery_code_hashes.c.consumed_at.is_(None))
                .values(consumed_at=_iso())
            )
            if (result.rowcount or 0) != 1:
    ...
    async def issue_session(
        ...
        *,
        ...
        now = _now()
        level = AuthenticationSecurityPolicy._login_level(methods)
        phishing = level in {"PASSKEY_UV", "ENTERPRISE_SSO"}
        async with self.engine.raw.begin() as conn:
            if rotate_from:
                await conn.execute(
                    update(web_sessions).where(web_sessions.c.token_hash == _hash(rotate_from)).values(revoked=1)
                )
            await conn.execute(
    ...
    async def load_session(self, raw_token: str) -> tuple[str, str, SessionAssurance]:
        ...
        async with self.engine.raw.begin() as conn:
            ...
            if idle and _iso(now) >= idle:
                raise unauthenticated()
            user = (await conn.execute(select(web_users).where(web_users.c.id == row.user_id))).fetchone()
            if user is None or user.disabled or user.verification_status == "SUSPENDED":
                raise unauthenticated()
            if row.org_id:
                membership = (
                ...
                org = (await conn.execute(select(organizations).where(organizations.c.id == row.org_id))).fetchone()
                if membership is None or getattr(membership, "status", "ACTIVE") != "ACTIVE":
                    raise unauthenticated()
                if org is None or getattr(org, "governance_status", "ACTIVE") in {"DISABLED", "SUSPENDED"}:
                    raise unauthenticated()
            await conn.execute(
                update(web_sessions)
            ...
            methods = tuple(json.loads(getattr(row, "auth_methods_json", None) or "[]") or ["PASSWORD"])
            assurance = SessionAssurance(
    ...
    async def list_sessions(self, user_id: str) -> list[dict[str, Any]]:
        async with self.engine.raw.connect() as conn:
            rows = (await conn.execute(select(web_sessions).where(web_sessions.c.user_id == user_id))).fetchall()
        return [
            {
    ...
    async def issue_step_up(self, session: SessionAssurance, action: SensitiveAction | str, *, org_id: str | None) -> str:
    ...
    async def consume_step_up(self, session: SessionAssurance, action: SensitiveAction | str, grant: str) -> None:
        async with self.engine.raw.begin() as conn:
            row = (
                await conn.execute(select(step_up_grants).where(step_up_grants.c.grant_hash == _hash(grant)))
            ).fetchone()
            if row is None:
            ...
            if row.action != str(action):
                raise EnterpriseError(STEP_UP_REQUIRED, "Step-up grant is bound to a different action.", 401)
            if row.user_id != session.user_id or row.session_id != session.session_id:
                raise EnterpriseError(STEP_UP_REQUIRED, "Step-up grant is bound to another session.", 401)
            if session.rank() < ASSURANCE_RANK.get(row.assurance_required, 0):
                raise EnterpriseError("AUTHENTICATION_ASSURANCE_TOO_LOW", "Assurance too low for this action.", 403)
            result = await conn.execute(
    ...
    async def require_step_up(self, session: SessionAssurance, action: SensitiveAction | str, *, org_id: str | None, grant: str | None = None) -> None:
        if not grant:
            issued = await self.issue_step_up(session, action, org_id=org_id)
            raise EnterpriseError(STEP_UP_REQUIRED, f"Fresh step-up authentication is required:{issued}", 401)
    ...
    async def authenticate_password(self, email: str, password: str, *, org_id: str | None = None) -> tuple[str, SessionAssurance] | None:
        ...
        decision = self.policy.evaluate_login(methods=(AuthMethod.PASSWORD,), role=role, org=org_policy)
        if not decision.allowed:
            raise EnterpriseError(decision.code, "Authentication policy denied this login.", 403)
        token, csrf, assurance = await self.issue_session(
            user_id=user_id, methods=(AuthMethod.PASSWORD,), org_id=resolved_org, ip_label=None, user_agent=None
        )
        return token, csrf, assurance

    async def google_login(self, *, id_token: str, nonce: str, intended_org_id: str | None = None) -> dict[str, Any]:
        ...
        if intended_org_id:
            ...
            if binding is None:
                raise EnterpriseError(COMPANY_VERIFICATION_REQUIRED, "Organization is not bound to Google Workspace.", 403)
            if kind != "WORKSPACE":
                raise EnterpriseError(GOOGLE_WORKSPACE_MISMATCH, "Personal Google accounts cannot join via company path.", 403)
            if claims.hosted_domain != binding["verified_domain"]:
                raise EnterpriseError(GOOGLE_WORKSPACE_MISMATCH, "Google hosted domain does not match the bound organization.", 403)
    ...
    async def microsoft_login(self, *, id_token: str, nonce: str, intended_org_id: str | None = None) -> dict[str, Any]:
        if not self.microsoft_client_id:
            raise forbidden(PROVIDER_TOKEN_INVALID, "Microsoft Sign-In is not configured.")
        header_iss = _unverified_iss(id_token)
        validator = OIDCTokenValidator(
            issuer=header_iss,
            audience=self.microsoft_client_id,
            jwks_url=self.microsoft_jwks_url if "login.microsoftonline.com" in self.microsoft_jwks_url else header_iss + "/discovery/v2.0/keys",
        )
        # Issuer is verified against the token after signature; pin to login.microsoftonline.com.
        claims = await validator.validate(id_token, expected_nonce=nonce)
        ...
        if intended_org_id:
            ...
            if binding is None:
                raise EnterpriseError(COMPANY_VERIFICATION_REQUIRED, "Organization is not bound to Entra.", 403)
            if kind == "PERSONAL":
                raise EnterpriseError(ENTRA_TENANT_MISMATCH, "Personal Microsoft accounts cannot use the organization path.", 403)
            if tenant != binding["tenant_id"]:
                raise EnterpriseError(ENTRA_TENANT_MISMATCH, "Entra tenant does not match the bound organization.", 403)
            return await self._complete_provider_login(
    ...
    async def _complete_provider_login(
        ...
        *,
        ...
        tenant_key = claims.tenant_id or claims.hosted_domain or ""
        async with self.engine.raw.connect() as conn:
            link = (
                await conn.execute(
                    select(provider_identities).where(
                        provider_identities.c.provider == provider,
                        provider_identities.c.subject == claims.subject,
                        provider_identities.c.tenant_id == (tenant_key or ""),
                    )
                )
            ).fetchone()
        if link is None:
        ...
        if org_id:
            await self._assert_membership(link.user_id, org_id)
        ...
        await self._notify("provider_login", user_id=link.user_id, org_id=org_id, payload={"provider": provider})
        return {"status": "AUTHENTICATED", "session_token": token, "assurance": assurance.level, "account_kind": account_kind}
    ...
    async def _assert_membership(self, user_id: str, org_id: str) -> None:
        ...
        if row is None:
            raise EnterpriseError(SSO_TENANT_MISMATCH, "Provider login does not grant organization membership.", 403)
    ...
    async def link_provider(
        ...
        *,
        ...
        action = SensitiveAction.LINK_GOOGLE if provider == GOOGLE_PROVIDER else SensitiveAction.LINK_MICROSOFT
        await self.require_step_up(session, action, org_id=session.org_id, grant=grant)
        tenant_key = claims.tenant_id or claims.hosted_domain or None
        try:
            async with self.engine.raw.begin() as conn:
                await conn.execute(
            ...
            raise EnterpriseError(ACCOUNT_LINK_CONFLICT, "Provider identity is already linked.", 409) from exc
    ...
    async def bind_organization_idp(
        ...
        *,
        ...
        if verification is None or verification.status not in {"DOMAIN_VERIFIED", "ORGANIZATION_VERIFIED"}:
            raise EnterpriseError(COMPANY_VERIFICATION_REQUIRED, "Company verification is required before IdP binding.", 403)
        async with self.engine.raw.begin() as conn:
            await conn.execute(
        ...
        await self._notify("idp_binding_changed", user_id=configured_by, org_id=org_id, payload={"provider": provider})
    ...
    async def configure_sso(
        ...
        *,
        ...
        await self.require_step_up(session, SensitiveAction.CONFIGURE_SSO, org_id=org_id, grant=grant)
        if enforcement == "SSO_OPTIONAL":
            existing = await self._org_policy(org_id)
            if existing.sso_enforcement == "SSO_REQUIRED":
                raise EnterpriseError(SECURITY_DOWNGRADE_BLOCKED, "Disabling required SSO needs dual control and phishing-resistant step-up.", 403)
        ...
        if issuer:
            validate_outbound_url(issuer if "://" in issuer else f"https://{issuer}", DestinationPolicy.PUBLIC_ONLY)
        async with self.engine.raw.begin() as conn:
            await conn.execute(
                insert(organization_sso_configs).values(
                    ...
                    provisioning=provisioning if provisioning in {"INVITE_ONLY", "JIT_OPT_IN"} else "INVITE_ONLY",
    ...
    async def consume_oidc_code_claims(
        ...
        *,
        ...
        async with self.engine.raw.connect() as conn:
            cfg = (
                await conn.execute(select(organization_sso_configs).where(organization_sso_configs.c.org_id == org_id))
            ).fetchone()
        if cfg is None:
        ...
        binding = await self._binding(org_id, ENTRA_PROVIDER) or await self._binding(org_id, GOOGLE_WORKSPACE)
        if binding and claims.tenant_id and binding.get("tenant_id") and claims.tenant_id != binding["tenant_id"]:
    ...
    async def start_domain_challenge(self, *, org_id: str, domain: str, method: str, session: SessionAssurance, grant: str | None) -> str:
        await self.require_step_up(session, SensitiveAction.CHANGE_COMPANY_DOMAIN, org_id=org_id, grant=grant)
    ...
    async def complete_domain_challenge(self, *, org_id: str, domain: str, method: str, token: str) -> None:
        async with self.engine.raw.begin() as conn:
            row = (
                await conn.execute(
            ...
            if method == "DNS_TXT":
                ...
                if body != token:
                    raise forbidden(COMPANY_VERIFICATION_REQUIRED, "HTTPS well-known challenge failed.")
            await conn.execute(
                update(company_domain_challenges).where(company_domain_challenges.c.id == row.id).values(
                    status="VERIFIED", verified_at=_iso()
                )
            )

    async def request_recovery(self, email: str) -> None:
        self.limiter.check(f"recovery:{email.casefold()}", limit=5, window_seconds=300)
        ...
        async with self.engine.raw.connect() as conn:
            user = (
                await conn.execute(select(web_users).where(web_users.c.email == email.strip().casefold(), web_users.c.disabled == 0))
            ).fetchone()
        if user is None:
            return
        privileged = False
    ...
    async def consume_recovery_token(
        ...
        *,
        ...
        self.limiter.check("recovery-token", limit=20, window_seconds=60)
        async with self.engine.raw.begin() as conn:
            row = (
                await conn.execute(
                    select(account_recovery_requests).where(account_recovery_requests.c.token_hash == _hash(token))
                )
            ).fetchone()
            ...
            if user_id and row.user_id != user_id:
                raise unauthenticated()
            if row.privileged and row.status == "RECOVERY_REVIEW":
                raise EnterpriseError(RECOVERY_REVIEW_REQUIRED, "Privileged recovery requires additional evidence.", 403)
            if row.privileged and not (recovery_code or passkey_ok):
                raise EnterpriseError(RECOVERY_REVIEW_REQUIRED, "OWNER recovery cannot proceed from email alone.", 403)
            result = await conn.execute(
                update(account_recovery_requests)
                .where(account_recovery_requests.c.id == row.id, account_recovery_requests.c.consumed_at.is_(None))
                .values(consumed_at=_iso(), status="RECOVERY_CONSUMED")
            )
            if (result.rowcount or 0) != 1:
                raise EnterpriseError(CHALLENGE_REPLAY, "Recovery token already used.", 401)
            await conn.execute(update(web_sessions).where(web_sessions.c.user_id == row.user_id).values(revoked=1))
            recovered_user = row.user_id
        await self._notify("password_reset", user_id=recovered_user)
        return recovered_user

    async def break_glass(self, *, org_id: str, user_id: str, session: SessionAssurance) -> dict[str, str]:
        org = await self._org_policy(org_id)
        if not org.break_glass_user_id or org.break_glass_user_id != user_id:
            raise EnterpriseError(BREAK_GLASS_DENIED, "Break-glass is not configured for this identity.", 403)
        if not session.phishing_resistant:
            raise EnterpriseError(PASSKEY_REQUIRED, "Break-glass requires phishing-resistant authentication.", 403)
        async with self.engine.raw.connect() as conn:
        ...
        if user is None or user.verification_status != "IDENTITY_VERIFIED":
            raise EnterpriseError(BREAK_GLASS_DENIED, "Break-glass identity must be IDENTITY_VERIFIED.", 403)
        if PRODUCTION_GATE_B_OPEN:
            raise RuntimeError("break-glass must not open Gate B")
        await self._notify("break_glass", user_id=user_id, org_id=org_id, payload={"severity": "high"})
        return {"status": "BREAK_GLASS_ACTIVE", "capabilities": "org_unlock_sso", "execution_authority": "denied"}

    async def evaluate_role_change(self, session: SessionAssurance, *, new_role: str, org_id: str) -> None:
        org = await self._org_policy(org_id)
        if new_role in org.privileged_roles and not session.phishing_resistant:
            raise EnterpriseError(PASSKEY_REQUIRED, "Privilege elevation requires phishing-resistant step-up.", 403)
