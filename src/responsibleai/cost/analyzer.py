...
_BLOAT_PATTERNS = [
    ...
    (
        r"(?:it is|it's)\s+(?:important|crucial|essential|vital)\s+to\s+(?:note|mention|remember)",
        "emphasis_filler",
    ),
]
...
class CostAnalyzer:
    ...
    def analyze_prompt_efficiency(
        ...
        if not pricing.is_local:
            ...
            if cheaper and cheaper.input_cost_per_million < pricing.input_cost_per_million * 0.5:
                savings_per_req = pricing.cost_for(tokens, resp_tokens) - cheaper.cost_for(
                    tokens, resp_tokens
                )
                ...
                findings.append(
                    WasteFinding(
                        ...
                        severity="medium"
                        if savings_per_req * 12 * monthly_requests > 100
                        else "low",
                        description=(
        ...
        optimized_tokens = max(
            ...
            tokens
            - sum(
                int(f.estimated_savings_usd / pricing.cost_for(1, 0))
                if pricing.cost_for(1, 0) > 0
                else 10
                for f in findings
    ...
    def governance_score(
        ...
        budget_score = max(
            0.0, 100.0 - max(0, total_cost_usd - monthly_limit_usd) / monthly_limit_usd * 100
        )
