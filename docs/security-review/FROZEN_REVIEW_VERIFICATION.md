...
> **CLOSED (2026-09-02, PM-authorized, option (b) only):** the trigger-scoping gap named above is fixed — see [`CI_GAP_ROOT_CAUSE_AND_FIX.md`](CI_GAP_ROOT_CAUSE_AND_FIX.md) for the full audit, fix, and verified result. At the current frozen SHA (`860c806c510d049ad53c94f8e3e449c0acf7265c`), real GitHub Actions CI has executed against PR #55 for the first time and **all 12 required/available checks pass** (CI lint/type/test matrix ×2 Python versions, Build distribution, Helm chart lint, Accessibility, i18n, DCO, Gitleaks, Dependency Review, CodeQL ×2 analyses + overall context). The finding this section originally documented is historically accurate for every SHA up to and including `54e9cb8`; it no longer describes the branch's current state.
