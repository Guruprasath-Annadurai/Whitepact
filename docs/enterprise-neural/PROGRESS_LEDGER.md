...
| 2 | Cryptographic Foundation + Key Management | **IN PROGRESS** — design audit PASS, implementation Step 1/5 PASS (`governance/crypto/` package, Protocol, LocalEnvelopeKeyProvider, envelope format); Steps 2-5 (migration, call-site wiring) remain | pending (uncommitted at ledger update time) | 47/47 new tests pass, 100% coverage; full suite 2922 passed, 0 failed | 2 real bugs found by property/unit tests and fixed pre-commit (KeyId sentinel collision, version-numbering overwrite bug) — see `02_PHASE2_REPORT.md` | No call site wired yet (dead code until Step 3); no persistent store; no concurrency locking designed yet |
