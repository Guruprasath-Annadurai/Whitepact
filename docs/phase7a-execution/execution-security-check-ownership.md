...
3. **Durable Immutable Execution Request:** Action arguments, purpose, and agent identity are immutably persisted in `runtime_execution_requests` prior to queueing. Table is append-only with zero mutable status.
4. **Atomic Issuance Transaction:** Request, authorization (`UNIQUE(approval_id)`), initial attempt (`PENDING`, NULL lease fields), and fence row (`current_generation=0`) are created in a single PostgreSQL transaction.
...
6. **One-Shot Backend Start Transition:** Possession of an `AdmissionReceipt` is insufficient to execute. Downstream execution requires `claim_backend_start()` to atomically verify lease generation, unexpired lease status, and transition `ADMITTED` to `BACKEND_STARTING` in `runtime_execution_attempts`.
7. **Downstream Executor Verification:** Downstream executors accept only `BackendExecutionClaim` and must verify it via `assert_backend_start_claim(claim)` before invoking containers or network sockets.
8. **Monotonic Worker Fencing:** Each lease acquisition increments `runtime_execution_fences.current_generation`. Stale/zombie workers fail closed.
9. **Universal Capacity Release:** Every capacity reservation made at enqueue time has an explicit terminal release path.
10. **Evidence Precedes Lease Finalization:** Backend result -> durable effect/outcome state -> durable evidence/audit write -> mark attempt terminal -> finalize lease -> release capacity.
...
| **One-Shot Backend-Start Claim** | `ExecutionAttemptRepository.claim_backend_start()` | NO | NO | YES (Atomic UPDATE: `ADMITTED -> BACKEND_STARTING` with `rowcount == 1`) | NO (Sole owner of backend-start ownership) | Verifies unexpired lease & generation under lock; emits `BackendExecutionClaim`. |
| **Durable Executor Claim Verification** | Downstream Executors (`InternalToolExecutor`, `UpstreamMCPExecutor`) | NO | NO | YES (`assert_backend_start_claim` against PostgreSQL) | NO (Executor gatekeeper) | Verifies attempt is in `BACKEND_STARTING` and bindings match prior to compute/network invocation. |
...
| **SafeNetwork Validation & IP Pinning** | `SafeNetworkBackend` | NO | NO | YES (Immediate pre-socket bind) | NO (Sole owner of network egress) | Enforces SSRF protection, IP pinning, redirect validation, and sets `effect_state = 'EFFECT_TRANSMITTING'`. |
...
| **Durable Evidence & Audit** | `EvidenceRepository` / `AuditLedger` | NO | NO | YES (Committed before lease finalization) | NO (Sole owner of compliance evidence) | Evidence is written before attempt completion. If evidence write fails after confirmed effect, marks `INCOMPLETE`. |
