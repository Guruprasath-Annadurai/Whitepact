...
| Path 8 (direct `dispatch_tool()` import) | **MITIGATED, not fully closeable** (Phase E5) — renamed to `_dispatch_tool_unchecked` with an explicit "no governance, internal only" docstring; `tests/test_dispatch_tool_unchecked_call_sites.py` fails the suite if a third call site (beyond `mcp/server.py`'s stdio path and `governance/execution.py`'s `InternalToolExecutor`) ever appears, and if it's ever re-exported from a package `__init__.py` or aliased back to a public-looking name. A caller with repo access can still import a private function directly — Python offers no stronger guarantee, and no wrapper function changes that; naming and durable audit enforcement are the actual, honest closure here. |
