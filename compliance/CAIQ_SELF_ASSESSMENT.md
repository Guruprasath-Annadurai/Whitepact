...
| Question | Answer |
|---|---|
| Are applications tested for security vulnerabilities before release? | Automated: ruff and mypy (strict-optional, zero suppressed errors) gate every commit via CI. `pip-audit` scans every dependency on every CI run — a default `pip install` carries zero known vulnerabilities; the one historical finding (nltk path traversal, PYSEC-2026-597) was resolved by moving nltk out of the mandatory dependency set entirely into an opt-in `[sentiment]` extra, since it's used in exactly one place with a hardcoded, non-attacker-controlled resource name — see Domain 15. Manual third-party pentest: not yet performed (see Domain 16). |
...
| Can security incidents be logged in a structured, exportable format? | Yes — `rai_incident_log` MCP tool produces structured incident records with SIEM-ready payloads (severity, SLA resolution targets, evidence hashing), and `POST /api/incidents` persists them into a real, queryable `incidents` table (`GET /api/incidents`, `GET /api/incidents/{id}`). Prometheus alerts also auto-create incident records via `POST /api/alerts/webhook` — see `grafana/prometheus/alertmanager.yml.example`. |
...
| Are security headers set (CSP, HSTS, X-Frame-Options)? | Yes. `SecurityHeadersMiddleware` sets `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `X-XSS-Protection`, `Referrer-Policy: strict-origin-when-cross-origin`, `Cache-Control: no-store`, `Permissions-Policy`, a scoped `Content-Security-Policy`, and `Strict-Transport-Security` on every response. **Honest caveat on the CSP**: `script-src`/`style-src` still need `'unsafe-inline'` because the dashboard's static HTML uses inline `<script>`/`<style>` blocks and `onclick=` attribute handlers rather than a build step with nonces — that's a real relaxation, not a hidden one, and it still blocks framing (`frame-ancestors 'none'`), object/embed (`object-src 'none'`), and restricts `connect-src`/`default-src` to same-origin. HSTS is also still recommended at the TLS-terminating reverse proxy (`DEPLOYMENT.md`'s nginx config) for defense in depth; the app-level header is a no-op over plain HTTP and doesn't replace that. |
