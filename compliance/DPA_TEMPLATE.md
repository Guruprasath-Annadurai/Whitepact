...
|---|---|---|---|---|
| **Render** | Compute hosting for the Processor-operated live reference deployment — builds and runs the application from source on every push | Application runtime only — request/response data passes through at runtime; no database data is stored on Render itself. | US region (Oregon) — see `compliance/CAIQ_SELF_ASSESSMENT.md` Domain 6 for full detail; free tier, no persistent local disk, no multi-region redundancy. | SOC 2 Type II compliant, ISO/IEC 27001 certified — see [render.com/docs/certifications-compliance](https://render.com/docs/certifications-compliance). |
| **Supabase** | Managed PostgreSQL database for the Processor-operated live reference deployment | All data stored by the platform when running on this infrastructure — trust scores, audit logs, cost/usage records, organization/API key metadata. Encrypted at rest by default as part of Supabase's certified infrastructure. | `aws-1-us-west-2` region, accessed via transaction-mode connection pooler — see `compliance/CAIQ_SELF_ASSESSMENT.md` Domain 6; free tier, no multi-region replication. | SOC 2 Type II, ISO 27001, HIPAA, PCI DSS certified — see [supabase.com/docs/guides/security/soc-2-compliance](https://supabase.com/docs/guides/security/soc-2-compliance). |
| **Upstash** | Managed Redis for the Processor-operated live reference deployment's rate-limit backend | Rate-limit counters only — no governance data, PII, or credentials. | Region selected at signup; free tier. | No independently verified certification found as of this review — stated honestly rather than assumed. |
...
- Encryption at rest (infrastructure-dependent — see Section 2's Supabase
  entry for the reference deployment's specifics)
