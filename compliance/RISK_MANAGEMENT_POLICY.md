...
| RM-002 | Protected-branch changes depend on required CI/DCO checks | Low | Verified: GitHub blocks merges while required status checks are not satisfied; DCO requires signed-off commits | Implemented control |
| RM-003 | Reference deployment lacks demonstrated cross-region failover | High | Disclosed in CAIQ/vendor-risk/continuity evidence | Accepted for current reference stage |
| RM-004 | Some vendor assurance remains inherited/shared rather than WhitePact-owned | Low | Data minimization and documented vendor boundaries | Monitor |
...
| RM-011 | Compliance evidence may drift away from the final production release | High | Require exact-SHA revalidation before enterprise launch claims | Open until final V1 release freeze |
| RM-012 | Legal/regulatory interpretation may be incomplete without qualified counsel | Medium | Internal mappings are labeled readiness evidence only; counsel sign-off remains external | Accepted / external dependency |
...
| 2026-09-13 | WhitePact assurance closure | Added exact-release evidence-drift and legal-interpretation risks for pre-audit closure. |
