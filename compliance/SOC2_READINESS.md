...
| Change management | Yes | Alembic migrations are version-controlled and production startup fails closed unless the schema is at the repository head; deployment manifests run an explicit migration job. CI gates every merge on the full test suite (2,900+ tests), mypy, ruff, and `helm lint`. |
...
| Control area | Implemented | Evidence |
|---|---|---|
| Notice and disclosure | Documented, not yet attorney-reviewed | `PRIVACY_POLICY.md` — content is complete and specific (defined retention periods, response timeframes, legal bases), but must be validated against the final production deployment and certified by counsel (see that document's provenance notice). |
