...
| **Render** (`whitepact-mcp-http` service, dashboard hosting) | Candidate hosted topology; availability is an open external gate as of 2026-08-27 | Founder's Render account | Restore and validate the deployment from `render.yaml`, then establish account recovery and a second admin/deploy path. Self-hosted installs are unaffected. |
