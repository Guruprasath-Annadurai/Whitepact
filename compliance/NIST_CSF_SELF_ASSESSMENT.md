...
| GV.SC — Supply chain risk managed | Partial | Dependencies declared with version constraints; `pip-audit` now scans every CI run (see CAIQ Domain 15). Vendor risk assessment for third-party services now exists (`compliance/VENDOR_RISK_ASSESSMENT.md`, covering GCP, Stripe, customer-owned OIDC/LLM vendors) — still a one-time, opportunistic write-up rather than a scheduled recurring process, which the document itself states plainly. |
